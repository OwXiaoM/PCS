...
class ShiftConv2d1(nn.Module):
    def __init__(self, inp_channels, move_channels, move_pixels):
        ...
        up_channels = (mid_channel - move_channels, mid_channel)
        down_channels = (mid_channel, mid_channel + move_channels)
        self.weight[up_channels[0]:up_channels[1], 0, 2, 1] = 1.0  ## up
        self.weight[down_channels[0]:down_channels[1], 0, 0, 1] = 1.0  ## down
        self.weight[0:mid_channel - move_channels, 0, 1, 1] = 1.0  ## identity
        self.weight[mid_channel + move_channels:, 0, 1, 1] = 1.0  ## identity
...
class RB(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(inplace=True),
        ...
        self.shift_conv = ShiftConv2d1(inp_channels=n_feat, move_channels=4, move_pixels=2)
    ...
    def forward(self, x):
        sf = self.shift_conv(x)
        # sf = self.shift_bi_features(x, 2, 4,'H')
        y = self.conv_first(sf)
        # y = self.conv_first(self.shift_conv(x))
        y = y + x
